...
def proc_all_nrt(proc_all=False):
    glider_paths = list(Path("/data/OG_nrt").glob("*"))
    glidermissions = []
    ...
    for glider, mission in glidermissions:
        glidermission_xml = Path(f"/home/usrerddap/erddap/content/parts/OG_nrt_{glider}_M{mission}.xml")
        if proc_all or not glidermission_xml.exists():
            _log.info(f"Add {glider} M{mission}")
            subprocess.check_call(['/usr/bin/bash', "/home/usrerddap/erddap/xml_edit/og_nrt.sh", str(glider), str(mission)])
        else:
            _log.info(f"No update needed to {glider} M{mission}")


if __name__ == '__main__':
    logf = f'/data/log/og_nrt.log'
