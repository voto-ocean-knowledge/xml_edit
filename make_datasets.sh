...
tail -q -n +2  parts/gliderad2cp_files.xml >> new_datasets.xml && echo "" >> new_datasets.xml
...
cat  parts/SB2121_20250905T0539_R.xml >> new_datasets.xml && echo "" >> new_datasets.xml
for f in parts/sailbuoy_delayed_*.xml; do cat "$f" >> new_datasets.xml && echo "" >> new_datasets.xml; done
for f in parts/OG_nrt_*.xml; do tail -q -n +2 "$f" >> new_datasets.xml && echo "" >> new_datasets.xml; done
