...
def update_doc(glider, mission, kind, og=False):
    """
    ...
    document_loc = f"/home/usrerddap/erddap/xml_edit/xml/{addendum}{kind}_{glider}_M{mission}.xml"
    tree = ET.parse(document_loc)
    root = tree.getroot()
    #correct_charset(root)
    # Update dataset name
    ds_name = f"{addendum}{kind}_{glider}_M{mission}"
...
if __name__ == '__main__':
    ...
    og = False
    if 'OG_' in args.kind:
        args.kind = args.kind.replace('OG_', '')
        og = True
    ...
    logging.basicConfig(filename=logf,
                        ...
                        level=logging.INFO,
                        datefmt='%Y-%m-%d %H:%M:%S')
    _log.info(f"Start add dataset {args.glider} M{args.mission} to xml")
    update_doc(args.glider, args.mission, args.kind, og=og)
    _log.info(f"Complete add dataset {args.glider} M{args.mission} to xml")
